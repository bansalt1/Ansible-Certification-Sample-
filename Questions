...
control.example.com server has password-less SSH access to all managed servers (using the root user). node1:4.example.com servers have a 5GB secondary /dev/sdb disk attached. There is a regular “user” created on all the servers.
...
(If you’re using the automated deployments, Ansible is already installed) Install the ansible package on the control node (including any dependencies) and configure the following:

Create a regular user automation with the password of devops. Use this user for all sample exam tasks. All playbooks and other Ansible configuration that you create for this sample exam should be stored in /home/automation/plays. Create a configuration file /home/automation/plays/ansible.cfg to meet the following requirements:
...
Create Ansible vault file /home/automation/plays/secret.yml. Encryption/decryption password is devops. Add the following variables to the vault:
...
The name of the repository is rpms.
The description of the repository is “This is the main repo file”.
Repositories are available to use from http://repo.ansi.example.com/BaseOS and http://repo.ansi.example.com/AppStream
...
{{ ansible_default_ipv4.address }} is the IP address of the managed node. Create a playbook /home/automation/plays/apache.yml that uses the role and runs on hosts in the webservers host group.
